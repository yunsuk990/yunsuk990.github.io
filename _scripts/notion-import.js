...
async function downloadImages(md, ftitle) {
  const regex = /!\[(.*?)\]\((.*?)\)/g;
  let index = 0;

  return md.replace(regex, async function (match, p1, p2) {
...
`;
    ...
    // 대체된 md는 비동기적으로 이미지 다운로드가 끝난 후 반환
    const edited_md = await downloadImages(md, ftitle);
